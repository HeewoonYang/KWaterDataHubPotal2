...
from app.models.user import UserAccount, UserRole, UserRoleMap, UserScreenPermission
...
class CurrentUser:
    ...
    def __init__(self, user: UserAccount, role_code: str, role_name: str,
                 role_group: str = "", screen_permissions: dict | None = None):
        self.id = user.id
        ...
        self.screen_permissions = screen_permissions or {}
...
async def get_current_user(
    ...
    # Redis 블랙리스트 체크 (로그아웃/세션 강제 만료 반영)
    jti = payload.get("jti")
    if jti and await is_revoked(jti):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="만료된 세션입니다")
    ...
    return CurrentUser(user, role_code, role_name, role_group or "", screen_permissions)
